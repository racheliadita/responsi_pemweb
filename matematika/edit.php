  <?php
    ...
    if (isset($_GET['edit'])){
      $SQL = $dbconnect->query ("SELECT * FROM mtk WHERE id=".$_GET['edit']);
      $getRow = $SQL ->fetch_array();
    }
    if(isset($_POST['update'])){
      $SQL = $dbconnect->prepare("UPDATE mtk SET nis=?, nama=?, kelas=?, nilai=? WHERE id=?");
  ...
  <html>
    <head>
      ...
      <link rel="stylesheet" type="text/css" href="mtkstyle.css">
    </head>

    <body>

      <div class="head">
        <h1><b>Edit Nilai Mata Pelajaran Matematika<b></h1>
      </div>

    <form class="form" action="" method="POST">
    <table align="center">
      <tr>
        <td><button formaction="../matematika/mtk.php">Back</button></td>
